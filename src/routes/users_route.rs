...
pub async fn get_user_by_username(
    ...
    match user {
        ...
        Err(_) => HttpResponse::NotFound().body("No user with this username"),
    }
}
pub async fn create_user(
    ...
    match check {
        Ok(user) => HttpResponse::InternalServerError().json(Message {
            message: "User with given username exist".to_owned(),
            status: None,
        }),
        Err(_) => {
            let hashed_password = hash(user_data.password.clone().expect("msg"), 10)
